...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	/**
	 * 所有的前后端交互的框架都是以servlet为基础的，所以在使用SpringMVC的时候，默认会把自己的容器设置成ServletContext
	 * 的属性，默认根容器的key为WebApplicationContext.Root,定义在WebApplicationContext中，所以在获取的时候只需要调用
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		// 此方式主要用于servlet3.0之后的环境（也就是说可以通过ServletContext.addServlet的方法注册servlet,
		// 此时就可以在创建FrameworkServlet和其子类的时候通过构造方法传递已经准备好的webApplicationContext）
		if (this.webApplicationContext != null) {
			// A context instance was injected at construction time -> use it
			wac = this.webApplicationContext;
			// 如果是ConfigurableWebApplicationContext类型，并且未激活，则进行初始化
		...
		// 此方式需要再配置Servlet的时候将servletContext中的webApplicationContext的name配置到contextAttribute属性就可以了
		/*
		    <servlet>
		...
		// 当前面两种方式都无效的情况下会创建一个webApplicationContext对象，一般情况下都是使用这种方式
		if (wac == null) {
	...
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		...
		// 如果不是ConfigurableWebApplicationConext类型，则抛出异常
		...
		// 获取contextConfigLocation属性，配置在servlet初始参数中
	...
	protected void doOptions(HttpServletRequest request, HttpServletResponse response)
		...
		// 如果 dispatchOptionsRequest 为true,则处理该请求，默认为true
	...
	protected void doTrace(HttpServletRequest request, HttpServletResponse response)
		...
		// 如果 dispatchTraceRequest 为true,则处理该请求，默认为false
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// 获取RequestContextHolder中原来保存的RequestAttribute(管理request和session的属性）
		...
		try {
			...
			// 发布ServletRequestHandlerEvent请求处理完成事件
